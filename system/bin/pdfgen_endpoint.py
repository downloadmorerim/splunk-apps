...
class PDFGenHandler(splunk.rest.BaseRestHandler):
            ...
            if self._viewType == self.VIEW_TYPE_DASHBOARD:
                ...
                if self._dashboardName != None:
                    (self._title, self._views) = pv.getDashboardTitleAndPanels(
                        dashboardName=self._dashboardName, 
    ...
    def _render(self):
        ...
        for i, view in enumerate(self._views):
            try:
                if view.requiresSearchJobObj() and view.getSearchJobObj() == None:
                    raise Exception("No search job available for view that requires search job")
        ...
        try:
            ...
            errorMsg = "Exception raised while trying to render \"%s\" to PDF. %s" % (self._title, e)
            import traceback
            logger.error("%s\n%s" % (errorMsg, traceback.format_exc()))
            self._outputError([errorMsg])
            return False
    ...
    def _startNextSearch(self):
        ...
        if view.getSearchJobObj() == None and view.requiresSearchJobObj():
            # start the search!
            logger.debug("dispatching search view: %s" % view.getTitle())
            if not view.dispatchSearch(overrideNowTime=self._now, maxRowsPerTable=self._maxRowsPerTable):
                logger.error("could not dispatch search view: %s" % view.getTitle())
    ...
    def _keepAllSearchesAlive(self):
        ...
        if self._touchSearchJobsLastTime != None:
    ...
    def _renderViewHeader(self, pdfRenderer, view):
        title = view.getTitle()
        if title != None and len(title) > 0:
            pdfRenderer.renderText("%s" % title, style = pdfRenderer.TITLE_STYLE, escapeText=True)
            pdfRenderer.spaceBetween(0.1 * pdfRenderer.ONE_INCH)
        description = view.getDescription()
        if description != None and len(description) > 0:
            pdfRenderer.renderText(description)
    ...
    def _renderTable(self, pdfRenderer, view):
        ...
        # get results object
        # SPL-80004, need to be defensive against a custom post-process that was added when rendering another visualization
        view.getSearchJobObj().setFetchOptions(search=None)
        results = view.getSearchJobResults()
        
        # determine field set
        explicitFieldList = view.getSearchParams().get(pu.SP_FIELD_LIST, [])
        fieldOrder = []
